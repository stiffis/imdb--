...
class GestorPeliculas {
  ...
  public:
    ...
    void recomendar_peliculas() {
        vector<pair<int, Pelicula>> peliculas_puntuadas; // Puntuación, Película
    
        // Mapa para contar géneros frecuentes en likes
        ...
        // Contar palabras frecuentes en historial
        ...
        // Puntuar todas las películas
        for (const auto& [id, pelicula] : peliculas) {
            if (likes.find(id) != likes.end()) continue; // Excluir ya likeadas
    
            int puntuacion = 0;
    
            // Puntuación por géneros coincidentes con likes
            for (const auto& etiqueta : pelicula.etiquetas) {
                if (generos_frecuentes.find(etiqueta) != generos_frecuentes.end()) {
                    puntuacion += generos_frecuentes[etiqueta] * 10; // 10 puntos por coincidencia
                }
            }
    
            // Puntuación por palabras del historial en título o sinopsis
            ...
            for (const auto& [palabra, freq] : palabras_historial) {
                ...
                puntuacion += ocurrencias * freq * 5; // 5 puntos por ocurrencia ponderada
            }
    
            // Pequeña puntuación aleatoria para exploración (0-5)
            puntuacion += rand() % 6;
        ...
        // Mostrar las 7 mejores recomendaciones
        cout << "\n=== Películas Recomendadas ===" << endl;
        int limite = min(7, static_cast<int>(peliculas_puntuadas.size()));
        if (limite == 0) {
            cout << "No hay suficientes datos para recomendar películas. ¡Agrega más likes o búsquedas!" << endl;
            return;
        }
...
int main() {
    srand(time(0)); // Semilla para el componente aleatorio
