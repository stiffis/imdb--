...
public:
    ...
    void buscar_pelicula(const string& termino) {
        ...
        // Esperar hasta tener al menos 5 resultados
        ...
        while (true) {
            ...
            if (respuesta == "si") {
                ...
                try {
                    ...
                    seleccion = -1;  // Si no se puede convertir, es inválido
                }
    ...
    void buscar_pelicula_worker(const regex& r) {
        // Limitar la búsqueda a los primeros 5 resultados
        for (const auto& [id, pelicula] : peliculas) {
            if (regex_search(pelicula.titulo, r) || regex_search(pelicula.sinopsis, r)) {
                ...
                if (resultados.size() >= 5) break;  // Detener la búsqueda después de encontrar 5 resultados
            }
        }
    }
};
